...
describe Events::AddTask do
  describe "GET /events/:event_id/tasks/new" do
    it "shows task form to organizer" do
      ...
      response = ApiClient.auth(organizer).exec(Events::AddTask.with(event.id))
    ...
    it "prevents non-organizers from accessing" do
      ...
      response = ApiClient.auth(other_user).exec(Events::AddTask.with(event.id))
  ...
  describe "POST /events/:event_id/tasks" do
    it "creates task with valid params" do
      ...
      response = ApiClient.auth(organizer).exec(Events::AddTask.with(event.id), task: {
        name:     "Bring desserts",
        guest_id: guest.id,
        category: "food",
    ...
    it "prevents non-organizers from creating tasks" do
      ...
      response = ApiClient.auth(other_user).exec(Events::AddTask.with(event.id), task: {
        name:     "Test Task",
        guest_id: guest.id,
      })
