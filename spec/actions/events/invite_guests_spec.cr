...
describe Events::InviteGuests do
  describe "GET /events/:event_id/invite" do
    it "shows invite page to organizer" do
      ...
      response = ApiClient.auth(organizer).exec(Events::InviteGuests.with(event.id), backdoor_user_id: organizer.id)
    ...
    it "prevents non-organizers from accessing" do
      ...
      response = ApiClient.auth(other_user).exec(Events::InviteGuests.with(event.id), backdoor_user_id: other_user.id)
  ...
  describe "POST /events/:event_id/invite" do
    it "creates guest invitations" do
      ...
      response = ApiClient.auth(organizer).exec(Events::InviteGuests.with(event.id), backdoor_user_id: organizer.id, user_ids: [user1.id.to_s, user2.id.to_s])
    ...
    it "prevents non-organizers from inviting" do
      ...
      response = ApiClient.auth(other_user).exec(Events::InviteGuests.with(event.id), backdoor_user_id: other_user.id, user_ids: [invite_user.id.to_s])
    ...
    it "shows success message" do
      ...
      response = ApiClient.auth(organizer).exec(Events::InviteGuests.with(event.id), backdoor_user_id: organizer.id, user_ids: [user.id.to_s])
