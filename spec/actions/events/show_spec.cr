...
describe Events::Show do
  it "shows event details to organizer" do
    ...
    response = ApiClient.auth(organizer).exec(Events::Show.with(event.id), backdoor_user_id: organizer.id)
  ...
  it "shows event details to invited guest" do
    ...
    response = ApiClient.auth(guest_user).exec(Events::Show.with(event.id), backdoor_user_id: guest_user.id)
  ...
  it "returns 404 for non-existent event" do
    ...
    expect_raises(Avram::RecordNotFoundError) do
      ApiClient.auth(user).exec(Events::Show.with(999999_i64), backdoor_user_id: user.id)
    end
  end
end
